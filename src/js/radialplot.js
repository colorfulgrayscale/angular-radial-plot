...
ui.radialplot = function() {
  function link(scope, element, attrs) {
    var radius = parseInt(attrs.plotRadius) || 43,
        ...
        angle, svg, line, increments, lineTween, incrementArc,
        scaleType = attrs.scale || 'linear', scale, increment = 20, areaCanvas,
        check = function(sum) {
            if(attrs.free === 'true') {
              return true;
            } else {
    ...
    scope.$watch('dsn', function(raw) {
      if (typeof raw === 'undefined' || inDrag) {
        return;
      }
          ...
          svg.selectAll('.label')
            ...
            .attr('transform', function(d,i) { 
              var transform = radius + padding,
                  x = (inner+radius+2) * Math.sin(angle(i-0.2)) + transform,
                  y = (-(inner+radius+2) * Math.cos(angle(i-0.2))) + transform,
